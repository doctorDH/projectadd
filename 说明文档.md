...
| 归档时间 | Change 名称 | 新增 Capability | 关键技术点 |
|---|---|---|---|
| 2026-02-27 | `oop-refactor` | `shell-core`, `command-system` | OOP 架构重构、LNK2019 修复 |
| 2026-02-27 | `basic-unix-commands` | `basic-unix-commands` | C++17 std::filesystem、UTF-8 编码修复 |
| 2026-02-27 | `advanced-unix-commands` | `advanced-unix-commands` | cat/cp/mv 实现、std::filesystem 文件操作 |
...
├── changes/
│   └── archive/
│       ├── 2026-02-27-oop-refactor/           ← Change 1 存档
│       ├── 2026-02-27-basic-unix-commands/     ← Change 2 存档
│       └── 2026-02-27-advanced-unix-commands/  ← Change 3 存档
└── specs/                                      ← 永久知识库（已同步 3 个 Change）
    ├── shell-core/spec.md
    ├── command-system/spec.md
    ├── basic-unix-commands/spec.md
    └── advanced-unix-commands/spec.md
```

### 全局 specs 知识库（当前状态）

**`specs/advanced-unix-commands/spec.md`** 新增（Change 3 产出）：

```
Capability 1: cat —— 5 个 Scenario（正常读取、中文、文件不存在、目标是目录、无参数）
Capability 2: cp  —— 4 个 Scenario（复制、覆盖、源不存在、参数不足）
Capability 3: mv  —— 5 个 Scenario（重命名、移动到目录、重命名目录、源不存在、参数不足）
```
...
*当前进度：Change 3 (advanced-unix-commands) — 全流程完成，已归档。累计完成 3 个 Change，4 个 Capability spec。*
